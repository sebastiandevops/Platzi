...
@click.option('-c', '--company', type=str, prompt=True,
              help='The client company')
@click.option('-e', '--email', type=str, prompt=True, help='The client email')
@click.option('-p', '--position', type=str, prompt=True,
...
def list(ctx):
    ...
    for client in clients_list:
        click.echo(('{uid}|  {name}  |  {company}  |  {email} '
                    '| {position}').format(
            uid=client['uid'], name=client['name'], company=client['company'],
